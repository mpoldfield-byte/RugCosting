...
export default function RugQuoteBuilder() {
  ...
  const exportExcel = async () => {
    ...
    const h2 = ['', '', '', '', '', '', '', '', '', ''];
    selectedProducts.forEach(() => h2.push('USD/SFT', 'USD/RUG', 'TOTAL'));
    rows.push(h2);
    ...
    rugsWithSft.forEach((r) => {
      ...
      selectedProducts.forEach((p) => {
        ...
        if (typeof price !== 'number') {
          ...
          row.push(price, Number((price * r.sft).toFixed(2)), Number((price * r.sft * (Number(r.qty) || 0)).toFixed(2)));
        }
      });
      rows.push(row);
    });
                  ...
                  <thead>
                    <tr>
                      ...
                      {selectedProducts.map((p) => (
                        <th key={p.name} className="product-head" colSpan={3}>
                          {p.name}
                        </th>
                      ))}
                    </tr>
                    <tr>
                      {selectedProducts.map((p) => (
                        <th key={`${p.name}-m`} className="product-head-mode" colSpan={3}>
                    ...
                    <tr>
                      {selectedProducts.map((p) => (
                        <React.Fragment key={`${p.name}-sub`}>
                          <th className="sub first">USD/SFT</th>
                          <th className="sub">USD/RUG</th>
...
function AttrRow({ label, cols, get }) {
  return (
    <tr className="attr-row">
      ...
      <td></td>
      {cols.map((p) => (
        <td key={`attr-${label}-${p.name}`} colSpan={3} className="rc">
          {get(p)}
        </td>
      ))}
    </tr>
